...
class LocationSensitiveAttention(nn.Module):
    """
    Applies a location-sensitive attention mechanism on the output features from the decoder.
    location-sensitive attention proposed in "Attention-Based Models for Speech Recognition" paper.
    The location-sensitive attention mechanism is performing well in speech recognition tasks.
...
class MultiHeadLocationSensitiveAttention(nn.Module):
    """
    Applies a multi-headed location-sensitive attention mechanism on the output features from the decoder.
    location-sensitive attention proposed in "Attention-Based Models for Speech Recognition" paper.
    The location-sensitive attention mechanism is performing well in speech recognition tasks.
    ...
    def __init__(self, hidden_dim: int, num_heads: int = 8, conv_out_channel: int = 10) -> None:
        super(MultiHeadLocationSensitiveAttention, self).__init__()
        self.hidden_dim = hidden_dim
    ...
    def forward(self, query: Tensor, value: Tensor, last_attn: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        loc_energy = (
            loc_energy.unsqueeze(1).repeat(1, self.num_heads, 1, 1).view(-1, seq_len, self.dim)
        )
